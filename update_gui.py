...
BUZZER_PIN = 23
PIR_PIN = 4
GREEN_LED_PIN = 18  # Added green LED pin
RED_LED_PIN = 27    # Added red LED pin
GPIO.setmode(GPIO.BCM)
GPIO.setup(SERVO_PIN, GPIO.OUT)
GPIO.setup(BUZZER_PIN, GPIO.OUT)
GPIO.setup(PIR_PIN, GPIO.IN)
GPIO.setup(GREEN_LED_PIN, GPIO.OUT)  # Set up green LED
GPIO.setup(RED_LED_PIN, GPIO.OUT)    # Set up red LED
...
def set_led_status(recognized=False):
    if recognized:
...
class FaceApp:
    def __init__(self, window):
        ...
        self.led_frame = tk.Frame(window, bg="#1e1e2f")
        self.led_frame.pack(pady=5)
        
        self.led_label = Label(self.led_frame, text="LED Status:", font=("Helvetica", 12), fg="white", bg="#1e1e2f")
        self.led_label.pack(side=tk.LEFT, padx=5)
        
        self.led_status = Label(self.led_frame, text="OFF", font=("Helvetica", 12, "bold"), fg="yellow", bg="#1e1e2f", width=8)
    ...
    def update_status(self, name):
        ...
        if name in ["Steven", "Mike", "Jay"]:
            self.led_status.config(text="GREEN", fg="green")
            set_led_status(recognized=True)
            self.log("💡 LED: GREEN - Authorized person")
        elif name == "Unknown":
            self.led_status.config(text="RED", fg="red")
            set_led_status(recognized=False)
            self.log("💡 LED: RED - Unknown person")
        elif name == "Camera Off":
            self.led_status.config(text="OFF", fg="yellow")
            turn_off_leds()
        else:
            self.led_status.config(text="RED", fg="red")
            set_led_status(recognized=False)
    ...
    def motion_watcher(self):
        while self.running:
            ...
            time.sleep(0.5)  # Adjust sensitivity by changing this
    ...
    def toggle_camera(self):
        ...
        if self.camera_active:
            ...
            turn_off_leds()  # Turn off LEDs when camera is off
            img = Image.fromarray(cv2.cvtColor(self.placeholder, cv2.COLOR_BGR2RGB))
            imgtk = ImageTk.PhotoImage(image=img)
            self.video_label.imgtk = imgtk
            self.video_label.configure(image=imgtk)
    ...
    def camera_loop(self):
        while self.running:
            ...
            if self.manual_mode == False:
                ...
                if self.detected_name in ["Steven", "Mike", "Jay"]:
                    set_servo_angle(90)
                    self.unknown_detected = False
                ...
                # If no face detected, set LED to red
    ...
    def on_close(self):
        self.running = False
        turn_off_leds()  # Make sure LEDs are off before closing
        GPIO.cleanup()
        self.window.quit()
